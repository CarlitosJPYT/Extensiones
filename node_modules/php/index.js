...
function run (script) {
  ...
  } catch (err) {throw new Error(`PHP process exited with code ${err.status}`)}
}
...
function runWithData (template, model) {
  ...
  if (typeof model._REGISTER_GLOBAL_MODEL === 'undefined') {
    if (settings.disableRegisterGlobalModel) {model._REGISTER_GLOBAL_MODEL = false}
     else {model._REGISTER_GLOBAL_MODEL = true}
    }
  ...
  try {const stdout = execSync(`php ${path.join(__dirname, '/loader.php')}`, {input: jsonModel})
    return stdout.toString()
  }
  catch (err) {
...
function disableRegisterGlobalModel () {settings.disableRegisterGlobalModel = true}
function enableRegisterGlobalModel () {settings.disableRegisterGlobalModel = false}
function circular (ref, methods) {
  ref = ref || '[Circular]'
  const seen = []
  return function (key, val) {
    if (typeof val === 'function' && methods) {val = val.toString()}
